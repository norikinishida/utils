...
def mkdir(path, newdir=None):
    ...
    if not os.path.exists(target):
        os.makedirs(target)
        writelog("Created a new directory: %s" % target)
...
def read_and_process_and_write(paths_in, paths_out, process: lambda line: line):
    """
    Parameters
    ----------
    paths_in: list[str]
...
def random_replace_list(xs, ps, z):
    ...
    if isinstance(ps, float):
        ps = np.zeros((batch_size,)) + ps
    else:
        assert len(ps) == batch_size
    rs = np.random.random((batch_size,))
...
############################
# Dictionary/JSON IO and manipuration
############################
...
############################
# Table/DataFrame IO and manipuration
############################
...
def build_vocabulary(paths_file,
                     path_vocab,
                     prune_at,
                     min_count,
                     special_words,
    ...
    process: function: str -> list[str] or None, default None
...
def convert_word2vec_to_weight_matrix(vocab, word2vec, dim, scale):
    ...
    writelog("Vocabulary size (pretrained): %d" % len(word2vec_vocab))
    writelog("Vocabulary size (shared): %d; |shared|/|task|: %.2f%% (%d/%d)" % \
                (len(shared_vocab),
                 float(len(shared_vocab))/len(task_vocab)*100.0,
                 len(shared_vocab), len(task_vocab)))
...
def normalize_vectors(mat):
    ...
    Parameters
    ----------
    mat: numpy.ndarray(shape=(batch_size, feat_dim))

    Returns
    -------
    numpy.ndarray(shape=(batch_size, feat_dim))
    """
...
def padding(xs, head, with_mask):
    ...
    Returns
    -------
    numpy.ndarray(shape=(batch_size, max_length))
    numpy.ndarray(shape=(batch_size, max_length))
    """
    batch_size = len(xs)
    max_length = max([len(x) for x in xs])
    ys = np.zeros((batch_size, max_length), dtype=np.int32)
    if head:
...
def transform_words(xs):
    ...
    Returns
    -------
    list[Variable(shape=(length,), dtype=np.int32)]
...
def convert_ndarray_to_variable(xs, seq):
    ...
    Parameters
    ----------
    xs: numpy.ndarray(shape=(batch_size, n_tokens))
    seq: bool

    Returns
    -------
    list[Variable(shape=(batch_size,))] or Variable(shape=(batch_size, n_tokens))
